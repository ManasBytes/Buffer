...
def write_workbook(output_path, question_df, basic_details_df):
	...
	question_sheet.column_dimensions["E"].width = 71
	for col_idx, col_name in enumerate(question_df.columns, start=1):
		if col_name == "Enter your answer":
			col_letter = question_sheet.cell(row=1, column=col_idx).column_letter
			question_sheet.column_dimensions[col_letter].number_format = "@"
