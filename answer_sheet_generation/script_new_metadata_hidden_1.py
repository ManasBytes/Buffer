...
def parse_subject_for_filename(subject_text):
	...
	if ":" in text:
		...
		return safe_filename_stem(combined)
...
def extract_subject_metadata(input_path):
	...
	subject_display_text = non_empty_values[0]
...
def write_workbook(output_path, question_df, basic_details_df):
	...
	for row_index, row_values in enumerate(basic_details_df.itertuples(index=False), start=1):
		...
		if row_values[0] in {"Subject", "DOB"}:
			value_cell.number_format = "@"
		if row_values[0] == "Session":
			session_dropdown = DataValidation(type="list", formula1='"AN,FN"', allow_blank=True)
			basic_sheet.add_data_validation(session_dropdown)
			session_dropdown.add(value_cell.coordinate)
	...
	for row_index, (_, row) in enumerate(question_df.iterrows(), start=2):
		for column_index, column_name in enumerate(question_df.columns, start=1):
			...
			if column_name == "Enter your answer":
				...
				cell.number_format = "@" 
			elif column_name in {"S.No", "Question Type", "Marks"}:
				cell.protection = Protection(locked=True)
	...
	question_sheet.column_dimensions["A"].hidden = True
	...
	question_sheet.column_dimensions["E"].width = 72
	answer_col_idx = next(
		(index for index, name in enumerate(question_df.columns, start=1) if name == "Enter your answer"),
		None,
	)
	if answer_col_idx is not None:
		answer_col_letter = question_sheet.cell(row=1, column=answer_col_idx).column_letter
		question_sheet.column_dimensions[answer_col_letter].number_format = "@"
